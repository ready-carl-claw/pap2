...
3. Read and migrate:
   - `PRD.md` → placeholder with legacy content (will be strengthened by LLM)
   - `Plan.md` → placeholder (will be regenerated by planning phase)
...
5. Report what was migrated and what still needs LLM processing

This command only works on projects with an existing `[PAP]` folder.
The result is a `[PAP2]` folder ready for PRD/Spec/Plan strengthening via LLM.
