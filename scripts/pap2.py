...
def llm_call_specialist(specialist_filename: str, task_prompt: str) -> str:
    specialist_path = Path(__file__).resolve().parent.parent / "specialists" / specialist_filename
    if not specialist_path.exists():
        raise RuntimeError(f"Specialist file not found: {specialist_path}")
    ...
    prompt = (
        ...
        "TASK:\n"
        f"{task_prompt}\n\n"
        "Return ONLY the requested markdown content without extra conversational text."
    )
    
    session_id = f"pap2-mig-{uuid.uuid4().hex[:8]}"
    cmd = [
        "openclaw", "agent",
    ...
    if res.returncode != 0:
        raise RuntimeError(f"LLM call failed: {res.stderr.strip() or res.stdout.strip()}")
        
    try:
        data = json.loads(res.stdout)
    except json.JSONDecodeError:
        raise RuntimeError(f"Invalid JSON from openclaw agent: {res.stdout}")
        
    payloads = data.get("result", {}).get("payloads", [])
    text = "\n".join(p.get("text", "") for p in payloads).strip()
    
    # Strip surrounding markdown code blocks if present
...
def cmd_migrate(args: argparse.Namespace) -> int:
    ...
    # 1. PRD.md - strengthened by LLM
    print(f"Strengthening PRD with product manager...", file=sys.stderr)
    prd_task = (
        "You are taking over an existing project. Read the legacy PRD and historical user steers below.\n"
        "Generate a complete, strengthened PRD.md following your specialist framework.\n"
        "Merge all the historical steering context into the core requirements so they are coherent.\n"
        f"Project Name: {project_name}\n\n"
        f"=== LEGACY PRD ===\n{legacy_prd}\n\n"
        f"=== HISTORICAL STEERS ===\n{legacy_steer}\n"
    )
    new_prd = llm_call_specialist("product-manager.md", prd_task)
    (pap2_root / "PRD.md").write_text(new_prd, encoding="utf-8")
    
    # 2. Spec.md - generated by software architect
    print(f"Generating Spec.md with software architect...", file=sys.stderr)
    spec_task = (
        "You are designing the system architecture for a project based on its newly updated PRD and legacy implementation plan.\n"
        "Generate a complete Spec.md following your specialist framework.\n"
        "Focus on the technical architecture, testable surfaces, and component boundaries.\n"
        f"Project Name: {project_name}\n\n"
        f"=== NEW PRD ===\n{new_prd}\n\n"
        f"=== LEGACY PLAN ===\n{legacy_plan}\n"
    )
    new_spec = llm_call_specialist("software-architect.md", spec_task)
    (pap2_root / "Spec.md").write_text(new_spec, encoding="utf-8")
    
    # 3. Plan.md - generated by senior project manager
    print(f"Generating Plan.md with project manager...", file=sys.stderr)
    plan_task = (
        "You are taking over an existing project. Read the PRD, Spec, and legacy progress/plan.\n"
        "Generate a complete Plan.md following your specialist framework.\n"
        "Break the work down into clear milestones with QA checkpoints.\n"
        f"Project Name: {project_name}\n\n"
        f"=== PRD ===\n{new_prd}\n\n"
        f"=== SPEC ===\n{new_spec}\n\n"
        f"=== LEGACY PROGRESS ===\n{legacy_progress}\n"
    )
    new_plan = llm_call_specialist("project-manager-senior.md", plan_task)
    (pap2_root / "Plan.md").write_text(new_plan, encoding="utf-8")
    ...
    if legacy_progress:
        # Avoid duplicating the header if it exists
        legacy_prog_lines = legacy_progress.splitlines()
        if legacy_prog_lines and legacy_prog_lines[0].startswith("# "):
            legacy_prog_lines = legacy_prog_lines[1:]
        progress_content += "\n".join(legacy_prog_lines).strip() + "\n"
    
    (pap2_root / "progress.md").write_text(progress_content, encoding="utf-8")
    
    # User-Steer.md - migrate and renumber
    steer_content = f"# {project_name} User Steer\n\n"
    if legacy_steer:
        steer_content += "## Active Log\n"
        steer_idx = 1
        for line in legacy_steer.splitlines():
            line = line.strip()
            ...
            steer_content += f"{steer_idx}. {'[Done] ' if is_done else ''}{line}\n"
            steer_idx += 1
    else:
        steer_content += "## Active Log\n"
    ...
    # TODO.md - migrate as YAML blocks
    todo_content = f"# {project_name} TODO\n\n"
    todo_content += "> Keep at most 5 active unfinished items.\n"
    todo_content += "> Task kinds: build, test, fix.\n\n"
    
    task_found = False
    if legacy_todo:
        task_idx = 1
        for line in legacy_todo.splitlines():
            m = re.match(r"^\s*-\s*\[([ xX])\]\s*(.*)$", line)
            if m:
                status = "done" if m.group(1).lower() in ["x"] else "open"
                title = m.group(2).strip().replace('"', '\\"')
                todo_content += (
                    "---\n"
                    f"id: task-00{task_idx}\n"
                    "kind: build\n"
                    f"title: \"{title}\"\n"
                    "milestone: M1\n"
                    "acceptance:\n"
                    "  - \"Migrated task completion\"\n"
                    f"status: {status}\n"
                    "---\n\n"
                )
                task_idx += 1
    ...
    result["note"] = "PRD/Spec/Plan fully generated via LLM. TODO reformatted into YAML blocks."
...
def cmd_finish_run(args: argparse.Namespace) -> int:
    ...
    if args.idle:
        state["consecutiveIdleRuns"] = int(state.get("consecutiveIdleRuns", 0)) + 1
    else:
    ...
    if args.summary_text:
        summary_msg = args.summary_text
        if instruction == "CRON_MUST_SHUTDOWN":
            summary_msg += "\n\n⚠️ **Autopilot disabled automatically** after 10 consecutive idle/blocked cycles to prevent token burn."
        append_summary(project_root, summary_msg, stamp=now_utc_stamp())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    finish_p.add_argument("--idle", action="store_true", help="Mark this run as blocked or idle (no meaningful work found). Stops autopilot after 10 consecutive idle runs.")
